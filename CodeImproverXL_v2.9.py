...
class CodeImprover:
    ...
    def __init__(self):
        ...
        self.llm.chatbot.new_conversation(
            system_prompt = prompts['default_rag_prompt'],
            modelIndex=2,
            switch_to = False
            )
        self.avs = AdvancedVectorStore(email=os.getenv("EMAIL"), password=os.getenv("PASSWD"))
        self.avs.logger.info("Init!")
        self.avs.load_vectorstore(self.avs.storage_dir)
        ...
        for path in paths:
            try: # Lets fetch the content 
                ...
                self.llm.switch_role(
                    system_prompt=system_prompt, model_id=2)
                ...
                results = self.llm.chatbot.query(user_task_prompt, web_search=True, stream=True) #  IN/OUT #
